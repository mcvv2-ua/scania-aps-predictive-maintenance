...
Para complementar esa visualización, la Tabla \ref{tab_outliers_iqr_resumen} resume algunas variables con mayor proporción de outliers según la regla IQR. Por este motivo, se añade la Tabla \ref{tab_outliers_iqr_resumen}, que resume algunas de las variables con mayor proporción de outliers según la regla IQR.
...
La reproducibilidad se apoya en tres decisiones. La primera es la separación de responsabilidades entre notebooks. Cada etapa produce artefactos que la siguiente puede cargar, lo que evita depender de resultados manuales. La segunda es el uso de funciones comunes en \texttt{project\_utils.py}. Este módulo define la carga del dataset, la conversión de etiquetas, el cálculo de métricas, la matriz de confusión y el coste. La tercera es la fijación de \texttt{RANDOM\_STATE = 42} en los experimentos.
